...
//-------------------- SPAWN BRIEFING
	...
	_briefing = "<t align='center'><t size='2.2'>Nouvelle Mission</t><br/><t size='1.5' color='#00B2EE'>Sécuriser la zone de l'épave</t><br/>____________________<br/>Un aéronef s'est écrasé dans cette zone.<br/>Partez à sa recherche et sécurisez la zone de l'épave...</t>";
	GlobalHint = _briefing; hint parseText GlobalHint; publicVariable "GlobalHint";
	showNotification = ["Nouvelle Mission", "Sécuriser la zone de l'épave"]; publicVariable "showNotification";
	UnderwaterMarkerText = "Sécuriser la zone de l'épave"; publicVariable "UnderwaterMarkerText";
	[west,["Underwaterintel"],["Un aéronef s'est écrasé dans cette zone.<br/>Partez à sa recherche et sécurisez la zone de l'épave...", "Sécuriser la zone de l'épave", "Sécuriser la zone de l'épave","UnderwaterMarker"],(getMarkerPos "UnderwaterMarker"),"Created",0,true,"intel",true] call BIS_fnc_taskCreate; 
...
for "_x" from 0 to (3 + (random 4)) do {
	...
	if !(_randomPos isEqualTo [0,0,0]) then {
		_infteamPatrol = [_randomPos, Independent, (configfile >> "CfgGroups" >> "Indep" >> "IND_F" >> "SpecOps" >> "HAF_DiverTeam")] call BIS_fnc_spawnGroup;
...
for "_x" from 0 to (random 3) do {
	...
	if !(_randomPos isEqualTo [0,0,0]) then {
		_boat = "I_Boat_Armed_01_minigun_F" createVehicle (_randomPos);
