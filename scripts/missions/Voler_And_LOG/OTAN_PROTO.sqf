...
private _PrototypeTypes = ["B_Plane_Fighter_01_F","B_Plane_CAS_01_dynamicLoadout_F","B_Plane_Fighter_01_Stealth_F","B_T_VTOL_01_vehicle_F","B_T_VTOL_01_infantry_F","B_T_VTOL_01_armed_F"];
...
for "_i" from 0 to _random do 
{
	...
	_spawnGroup = [_nposition, EAST, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfTeam_AT")] call BIS_fnc_spawnGroup;
...
{
	_nposition = [_fuzzyPos,random 50,250, 1, 0, 60 * (pi / 180), 0, []] call BIS_fnc_findSafePos;
	_spawnGroup_squad = [_nposition, EAST, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfSquad_Weapons")] call BIS_fnc_spawnGroup;
...
{
	_nposition = [_fuzzyPos,random 50,250, 1, 0, 60 * (pi / 180), 0, []] call BIS_fnc_findSafePos;
	_spawnGroup_squad = [_nposition, EAST, (configfile >> "CfgGroups" >> "West" >> "BLU_F" >> "Infantry" >> "BUS_InfTeam")] call BIS_fnc_spawnGroup;
	[_spawnGroup_squad, getpos PROTOOBJ,50 + random 200] call BIS_fnc_taskPatrol;
	PROTOunits_squad = PROTOunits_squad + (units _spawnGroup_squad);
	...
	if (_infBuildingAmount > 0) then {
		...
		#define PLANE_TYPE "B_Plane_Fighter_01_F","B_Plane_CAS_01_dynamicLoadout_F","B_Plane_Fighter_01_Stealth_F"
		...
		_pilotguy = [[0,0,0], EAST, ["B_Fighter_Pilot_F"],[],[],[],[],[],232] call BIS_fnc_spawnGroup;
