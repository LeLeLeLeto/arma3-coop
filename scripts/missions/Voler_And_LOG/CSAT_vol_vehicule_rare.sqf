...
//-------------------- SPAWN BRIEFING
	...
	_briefing = "<t align='center'><t size='2.2'>Nouvelle Mission</t><br/><t size='1.5' color='#00B2EE'>Voler le véhicule</t><br/>____________________<br/>Les forces CSAT protègent un véhicule de valeur dans cette zone.<br/>Partez à sa recherche et ramenez-le à voter base pour pouvoir l'utiliser...</t>";
...
{
	_nposition = [_fuzzyPos,random 50,250, 1, 0, 60 * (pi / 180), 0, []] call BIS_fnc_findSafePos;
	_spawnGroup_squad = [_nposition, EAST, (configfile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OIA_InfSquad")] call BIS_fnc_spawnGroup;
...
{
	_nposition = [_fuzzyPos,random 50,250, 1, 0, 60 * (pi / 180), 0, []] call BIS_fnc_findSafePos;
	_spawnGroup_squad = [_nposition, EAST, (configfile >> "CfgGroups" >> "East" >> "OPF_F" >> "Infantry" >> "OIA_ReconSquad")] call BIS_fnc_spawnGroup;
	[_spawnGroup_squad, getpos VolVehiculeObj,50 + random 200] call BIS_fnc_taskPatrol;
	VolVehiculeunits_squad = VolVehiculeunits_squad + (units _spawnGroup_squad);
...
private _enemiesArray = [objNull];
	...
	sleep 0.1;
	 ...
	 _infunits = ["O_Soldier_F","O_officer_F","O_Soldier_lite_F","O_Soldier_GL_F","O_Soldier_AR_F","O_soldier_M_F","O_Soldier_LAT_F","O_medic_F","O_engineer_F"];
