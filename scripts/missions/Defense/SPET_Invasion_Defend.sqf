...
//-------------------- SPAWN HELIDROP
		...
		["DEFENSECircle",5,true,false,2000, "random", true, 150, 100, 10, 0.01, 75,false] execVM "scripts\HeliDrop.sqf";
		["DEFENSECircle",5,true,false,2000, "random", true, 125, 100, 10, 0.01, 75,false] execVM "scripts\HeliDrop.sqf";
		["DEFENSECircle",5,true,false,2000, "random", true, 100, 100, 10, 0.01, 75,false] execVM "scripts\HeliDrop.sqf";
...
while { DEFENSEMissionUp } do {
	...
	if (_aliveInZone < 5) then
	{
		...
		sleep 3;
		{{deleteVehicle _x} forEach units _x; deleteGroup _x} forEach(allGroups select {side _x == EAST and {leader _x distance getMarkerPos "DEFENSEMarker" < 500}});
		sleep 3;
