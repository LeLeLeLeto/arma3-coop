...
//-------------------- SPAWN BRIEFING
	...
	_marqueur = createMarker ["marqueur_attaque_ldf", _fuzzyPos];
	_marqueur setMarkerType "selector_selectedMission";
	"marqueur_attaque_ldf" setMarkerSize [2, 2];
	"marqueur_attaque_ldf" setMarkerColor "ColorBlue";
...
{
	_nposition = [_fuzzyPos,random 50,250, 1, 0, 60 * (pi / 180), 0, []] call BIS_fnc_findSafePos;
	_spawnGroup_squad = [_nposition, Independent, (configfile >> "CfgGroups" >> "Indep" >> "IND_E_F" >> "Infantry" >> "I_E_InfTeam")] call BIS_fnc_spawnGroup;
...
while { INVASIONMissionUp } do {
	...
	if (_aliveInZone < 10) then
	{
		...
		// _aoUnderAttack = [] execVM "mission\Defense\LDF_Invasion_Defend.sqf";
		sleep 8;		
		...
		deleteMarker "marqueur_attaque_ldf";
	};	
};