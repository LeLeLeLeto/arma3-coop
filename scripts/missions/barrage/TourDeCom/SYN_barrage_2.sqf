...
private "_pos";
private "_nearRoad";
    _pos = [getMarkerPos "Destroy_And_EliminateCircle", 1500, 1500, 10, 0, 10, 0]  call BIS_fnc_findSafePos;
    _nearRoad = [_pos, 500] call BIS_fnc_nearestRoad;


//----Spawn obj
private _objective = selectRandom ["Land_Barricade_01_10m_F"];
BARRAGEObj_2 = createVehicle [_objective, _nearRoad, [], 0, "CAN_COLLIDE"];
BARRAGEBidonObj_2 = createVehicle ["MetalBarrel_burning_F", _nearRoad modelToWorld [0,2,0], [], 0, "CAN_COLLIDE"];
BARRAGEBidon2Obj_2 = createVehicle ["MetalBarrel_burning_F", _nearRoad modelToWorld [0,-2,0], [], 0, "CAN_COLLIDE"];
...
private _enemiesArray = [objNull];
	...
	//=====defining vehicles=========
	private _Randomvehicle = ["I_C_Offroad_02_LMG_F"];	

		//---------- RANDOM VEHICLE 
	private _vehpatrolgroupamount = 0;
	
	for "_i" from 0 to (1 + (random 1)) do {
		private _randomPos = [_nearRoad, 10, 10, 10, 0, 0.3, 0, [], (getPos BARRAGEObj_2)] call BIS_fnc_findSafePos;
	...
	//=====defining vehicles=========
private _Randomvehicle = ["I_static_AA_F","I_static_AT_F","I_HMG_01_high_F"];

//vehicles
private _RandomVicAmount = 0;
for "_x" from 1 to 3 do {
	private _randomPos = [_nearRoad, 15, 15, 15, 0, 0.3, 0, [], (getPos BARRAGEObj_2)] call BIS_fnc_findSafePos;
...
//-------------------- [ CORE LOOPS ] ------------------------ [ CORE LOOPS ]
	...
	_trg = createTrigger ["EmptyDetector", (getMarkerPos "Destroy_And_EliminateCircle")];
...
while { BARRAGEMissionUp } do {
	...
	if (_aliveInZone < 1) then
	{
		...
		BARRAGEunits_veh = [];
		BARRAGEunits_squad = [];
		deleteVehicle BARRAGEObj_2;
		deleteVehicle BARRAGEBidonObj_2;
		deleteVehicle BARRAGEBidon2Obj_2;
