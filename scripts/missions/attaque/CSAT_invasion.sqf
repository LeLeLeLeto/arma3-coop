...
//-------------------- SPAWN BRIEFING
	...
	_marqueur = createMarker ["marqueur_attaque_csat", _fuzzyPos];
	_marqueur setMarkerType "selector_selectedMission";
	"marqueur_attaque_csat" setMarkerSize [2, 2];
	"marqueur_attaque_csat" setMarkerColor "ColorBlue";

//-------------------- SPAWN FORCE PROTECTION

_random = (4 + floor(random(2)));

_escouades = ["OIA_InfTeam", "OIA_InfSquad", "OIA_ReconSquad"];
...
while { INVASIONMissionUp } do {
	...
	if (_aliveInZone < 10) then
	{
		...
		// _aoUnderAttack = [] execVM "mission\Defense\CSAT_Invasion_Defend.sqf";
		sleep 8;		
		...
		{deletevehicle _x; sleep 0.1;} foreach INVASIONunits;
		{deletevehicle _x; sleep 0.1;} foreach INVASIONunits_veh;
		{deletevehicle _x; sleep 0.1;} foreach INVASIONunits_squad;
		INVASIONunits = [];
		INVASIONunits_veh = [];
		INVASIONunits_squad = [];
		...
		deleteMarker "marqueur_attaque_csat";
	};	
};