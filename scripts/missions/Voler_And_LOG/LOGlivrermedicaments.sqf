...
private ["_x","_object","_object2","_object3","_object4","_briefing","_enemiesArray","_fuzzyPos","_completeText","_failedText"];
LOGunits = [];
LOGunits_veh = [];
LOGunits_squad = [];
...
//-------------------- BRIEFING
	...
	Voler_And_LOGMarkerText = "Livrer des fournitures médicales"; publicVariable "Voler_And_LOGMarkerText";
	"Voler_And_LOGMarker" setMarkerText "Livrer des fournitures médicales"; publicVariable "Voler_And_LOGMarker";
	publicVariable "LOGObj";
	_briefing = "<t align='center'><t size='2.2'>Nouvelle Mission</t><br/><t size='1.5' color='#00B2EE'>Livrer des fournitures médicales</t><br/>____________________<br/>Des civils nous ont signalé la présence de forces hostiles dans cette zone.<br/>C'est l'occasion de lâcher quelques balles...Rendez-vous sur zone et libérer la ville.</t>";
	GlobalHint = _briefing; hint parseText GlobalHint; publicVariable "GlobalHint";
	showNotification = ["Nouvelle Mission", "Livrer des fournitures médicales"]; publicVariable "showNotification";
	Voler_And_LOGMarkerText = "Livrer des fournitures médicales"; publicVariable "Voler_And_LOGMarkerText";
	
	[west,["LIVRERFOURNITURES"],["Livrer des fournitures médicales dans cette zone.<br/>Livrer des fournitures médicales...Rendez-vous sur zone et Livrer des fournitures médicales.", "Livrer des fournitures médicales","Voler_And_LOGMarker"],(getMarkerPos "Voler_And_LOGMarker"),"Created",0,true,"help",true] call BIS_fnc_taskCreate;
...
//-------------------- [ CORE LOOPS ] ------------------------ [ CORE LOOPS ]
	...
	if ({alive _x} count [_object,_object2,_object3,_object4] < 4) exitWith {
		...
		_failedText = format["<t align='center'><t size='2.2'>Mission</t><br/><t size='1.5' color='#FF0000'>ECHOUEE</t><br/>____________________<br/>Fournitures détruites!<br/>Continuez votre progression sur l'île en attendant d'autres objectifs...</t>"];
		...
		_null = ["LIVRERFOURNITURES", "FAILED"] spawn BIS_fnc_taskSetState;
		sleep 5;
		["LIVRERFOURNITURES"] call BIS_fnc_deleteTask; 
	...
	if (({alive _x} count [_object,_object2,_object3,_object4] > 0) && (_object distance getMarkerPos "Voler_And_LOGMarker" < 50) && (_object2 distance getMarkerPos "Voler_And_LOGMarker" < 50) && (_object3 distance getMarkerPos "Voler_And_LOGMarker" < 50) && (_object4 distance getMarkerPos "Voler_And_LOGMarker" < 50)) then
	{
	LOG_SUCCESS = true; publicVariable "LOG_SUCCESS";
		...
		showNotification = ["CompletedSideMission", Voler_And_LOGMarkerText]; publicVariable "showNotification";
		{ _x setMarkerPos [-10000,-10000,-10000]; } forEach ["Voler_And_LOGMarker", "Voler_And_LOGCircle"]; publicVariable "Voler_And_LOGMarker";
		_null = ["LIVRERFOURNITURES", "SUCCEEDED"] spawn BIS_fnc_taskSetState;
		sleep 5;
		["LIVRERFOURNITURES"] call BIS_fnc_deleteTask; 
