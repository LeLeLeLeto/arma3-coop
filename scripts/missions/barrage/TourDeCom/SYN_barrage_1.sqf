...
private "_pos";
private "_nearRoad";
    _pos = [getMarkerPos "Destroy_And_EliminateCircle", 1000, 1000, 10, 0, 10, 0]  call BIS_fnc_findSafePos;
    _nearRoad = [_pos, 500] call BIS_fnc_nearestRoad;


//----Spawn obj
private _objective = selectRandom ["Land_Barricade_01_10m_F"];
BARRAGEObj_1 = createVehicle [_objective, _nearRoad, [], 0, "CAN_COLLIDE"];
BARRAGEBidonObj_1 = createVehicle ["MetalBarrel_burning_F", _nearRoad modelToWorld [0,2,0], [], 0, "CAN_COLLIDE"];
BARRAGEBidon2Obj_1 = createVehicle ["MetalBarrel_burning_F", _nearRoad modelToWorld [0,-2,0], [], 0, "CAN_COLLIDE"];
...
private _enemiesArray = [objNull];
	...
	for "_i" from 0 to (1 + (random 1)) do {
		private _randomPos = [_nearRoad, 10, 10, 10, 0, 0.3, 0, [], (getPos BARRAGEObj_1)] call BIS_fnc_findSafePos;
...
for "_x" from 1 to 3 do {
	private _randomPos = [_nearRoad, 15, 15, 15, 0, 0.3, 0, [], (getPos BARRAGEObj_1)] call BIS_fnc_findSafePos;
...
while { BARRAGEMissionUp } do {
	...
	if (_aliveInZone < 1) then
	{
		...
		BARRAGEunits_veh = [];
		BARRAGEunits_squad = [];
		deleteVehicle BARRAGEObj_1;
		deleteVehicle BARRAGEBidonObj_1;
		deleteVehicle BARRAGEBidon2Obj_1;
